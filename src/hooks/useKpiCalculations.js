...
import { detectSectorAnomalies } from "../utils/anomalyDetector";
import { getBrand, getDeviceType } from "../utils/deviceClassifier";
...
export function useKpiCalculations({
  ...
  const filteredCases = useMemo(() => {
    ...
    return allCases.reduce((acc, c) => {
      if (!c.date || c.date < start || c.date > end) return acc;
  ...
  const displayCases = useMemo(() => {
    let output = filteredCases;
    if (selectedCategory) {
      output = output.filter(
        (c) => getCategory(mapType(c.type)) === selectedCategory,
      );
    }
    if (drillDownLabel) {
      output = output.filter((c) => {
        ...
        if (granularity === "year") cLabel = `${y}年`;
          ...
          cLabel = `${y}-Q${Math.ceil(m / 3)}`;
        else cLabel = `${y}-${String(m).padStart(2, "0")}`;
        return cLabel === drillDownLabel;
      });
    }
    return output;
  }, [filteredCases, selectedCategory, drillDownLabel, granularity]);
  ...
  const stats = useMemo(() => {
    ...
    let strat = {
      ...
      laborCost: 0,
      warrantyCount: 0,
      warMaintenanceCount: 0,
      tatOutliers: 0,
      totalPending: 0,
      totalBacklog: 0,
      totalConst: 0,
      warRepairTotal: 0,
      warRepairTypes: { 一般維修: 0, 困難維修: 0, 外修判定: 0, 簡易檢測: 0 },
      warRepairBrands: {},
      warAssetClass: {
      ...
      categories: {
        ...
        [TICKET_CATEGORIES.INSTALLATION]: {
          cases: 0,
          points: 0,
          revenue: 0,
          partsCost: 0,
        ...
        [TICKET_CATEGORIES.REFURBISHMENT]: {
          cases: 0,
          points: 0,
          revenue: 0,
          partsCost: 0,
    ...
    displayCases.forEach((c) => {
      ...
      const laborCost = (c.points || 0) * KpiConfig.laborCostPerPoint;
      ...
      if (strat.categories[cat]) {
        ...
        if (!strat.categories[cat].brands) strat.categories[cat].brands = {};
        const brand = getBrand(c.model);
        strat.categories[cat].brands[brand] =
          (strat.categories[cat].brands[brand] || 0) + 1;

        if (!strat.categories[cat].deviceTypes)
          strat.categories[cat].deviceTypes = {};
        const dt = getDeviceType(c.model);
        strat.categories[cat].deviceTypes[dt] =
          (strat.categories[cat].deviceTypes[dt] || 0) + 1;

        if (!strat.categories[cat].subTypes)
          strat.categories[cat].subTypes = {};
        const mappedT = mapType(c.type);
        strat.categories[cat].subTypes[mappedT] =
          (strat.categories[cat].subTypes[mappedT] || 0) + 1;
      }
      ...
      if (c.warranty) {
        if (cat === TICKET_CATEGORIES.MAINTENANCE) {
          ...
          strat.warAssetClass[assetClass].salesPersons[sp] =
            (strat.warAssetClass[assetClass].salesPersons[sp] || 0) + 1;
          ...
          if (
            ...
            const brand = getBrand(c.model);
            strat.warRepairBrands[brand] =
              (strat.warRepairBrands[brand] || 0) + 1;

            const dt = getDeviceType(c.model);
            strat.warRepairDeviceTypes = strat.warRepairDeviceTypes || {};
            strat.warRepairDeviceTypes[dt] =
              (strat.warRepairDeviceTypes[dt] || 0) + 1;
          }
        }
      }
      ...
      if (c.model && c.model !== "-" && c.model !== "") {
        const isExcluded = ["保養", "整新", "裝機", "安裝", "其他"].some((k) =>
          t.includes(k),
        );
        if (!isExcluded)
    ...
    const avgBacklog = total.cases
      ? (strat.totalBacklog / total.cases).toFixed(1)
      : "0";
    const avgConst = total.cases
      ? (strat.totalConst / total.cases).toFixed(1)
      : "0";
    const recallRate = total.recallDenom
      ? (total.recallNum / total.recallDenom) * 100
      : 0;
    const slaRate = total.cases
      ? ((strat.tatOutliers / total.cases) * 100).toFixed(1)
      : "0";
    const repairDenom =
      total.cases -
      (strat.categories[TICKET_CATEGORIES.MAINTENANCE]?.cases || 0);
    const warRate =
      repairDenom > 0
        ? ((strat.warrantyCount / repairDenom) * 100).toFixed(1)
        : "0";
    const sortedModels = Object.entries(strat.models)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5);
    const sortedParts = Object.entries(strat.parts)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10);
    const sortedEng = Object.values(engStats).sort(
      (a, b) => b.points - a.points,
    );

    const costWeightedParts = Object.entries(strat.parts)
      .map(([key, count]) => {
    ...
    const slaTieredArr = Object.entries(slaTiered)
      .map(([type, d]) => ({
        type,
        ...d,
        rate: d.total ? ((d.over / d.total) * 100).toFixed(1) : "0",
      }))
      .sort((a, b) => parseFloat(b.rate) - parseFloat(a.rate));
    const slaTieredOverall = total.cases
      ? displayCases.filter((c) => c.tat > getSlaTarget(mapType(c.type))).length
      : 0;

    const costPerRepair = total.cases
      ? ((strat.extCost + strat.partsCost) / total.cases).toFixed(0)
      : 0;
    const costByType = {};
    displayCases.forEach((c) => {
      const mt = mapType(c.type);
    ...
    const costByTypeArr = Object.entries(costByType)
      .map(([type, d]) => ({
        type,
        count: d.count,
        avg: d.count ? Math.round(d.cost / d.count) : 0,
        total: Math.round(d.cost),
      }))
    ...
    const mtbfData = Object.entries(modelSN)
      .map(([model, sns]) => {
        let totalGaps = 0,
          gapCount = 0,
          uniqueSN = Object.keys(sns).length,
          totalCases = 0;
        ...
        const failRate =
          uniqueSN > 0 ? parseFloat((totalCases / uniqueSN).toFixed(1)) : 0;
        return { model, uniqueSN, totalCases, mtbf, failRate };
      })
      .filter((d) => d.totalCases >= 2)
      .sort((a, b) => b.failRate - a.failRate)
      .slice(0, 10);

    const dateSpread = displayCases
      .filter((c) => c.date)
      .map((c) => c.date.getTime());
    let pDateMin = Infinity,
      pDateMax = -Infinity;
    ...
    const monthSpan =
      dateSpread.length > 1
        ...
        : 1;
    const partsInventory = Object.entries(strat.parts)
      .map(([key, count]) => {
        const pNo = key.split("||")[0].trim().toUpperCase();
        ...
        const safetyStock = Math.ceil(
          monthlyRate * KpiConfig.safetyStockMultiplier,
        );
        return {
          key,
          pNo,
    ...
    displayCases.forEach((c) => {
      ...
      skillMatrix[c.engineer][c.model] =
        (skillMatrix[c.engineer][c.model] || 0) + 1;
    });
    const modelCountMap = {};
    displayCases.forEach((c) => {
      if (c.model && c.model !== "-")
        modelCountMap[c.model] = (modelCountMap[c.model] || 0) + 1;
    });
  ...
  const monthlyTrends = useMemo(() => {
    ...
    allCases.forEach((c) => {
      ...
      if (!isExtRepairTrend) months[key].partsCost += pc;
      const t = c.type || "";
    ...
    return {
      ...
      avgTat: last6.map(([, v]) =>
        v.cases ? parseFloat((v.tatSum / v.cases).toFixed(1)) : 0,
      ),
      recallRate: last6.map(([, v]) =>
        v.recallDenom
          ? parseFloat(((v.recallNum / v.recallDenom) * 100).toFixed(1))
          : 0,
      ),
  ...
  const historicalStats = useMemo(() => {
    ...
    const curStart = new Date(dateRange.start);
    curStart.setHours(0, 0, 0, 0);
    const curEnd = new Date(dateRange.end);
    ...
    const getAggregateForPeriod = (sDate, eDate) => {
      ...
      return {
        ...
        slaRate: casesCount
          ? parseFloat(((tatOutliers / casesCount) * 100).toFixed(1))
          : 0,
      };
    };
    ...
    const momStart = new Date(
      momEnd.getTime() - diffDays * 24 * 60 * 60 * 1000 + 1,
    );
    const mom = getAggregateForPeriod(momStart, momEnd);
    const qoqEnd = new Date(curEnd);
    qoqEnd.setDate(qoqEnd.getDate() - 90);
    const qoqStart = new Date(curStart);
    qoqStart.setDate(qoqStart.getDate() - 90);
    const qoq = getAggregateForPeriod(qoqStart, qoqEnd);
    const yoyEnd = new Date(curEnd);
    yoyEnd.setFullYear(yoyEnd.getFullYear() - 1);
    const yoyStart = new Date(curStart);
    ...
    return {
      ...
      mom: {
        ...
        deltas: {
          ...
          slaRate: current.slaRate - mom.slaRate,
        },
      },
      qoq: {
        ...qoq,
  ...
  const sectorAnomalies = useMemo(() => {
    ...
    const curEnd = new Date(dateRange.end);
    curEnd.setHours(23, 59, 59, 999);
    const diffMs = curEnd - curStart;
    if (diffMs <= 0) return [];
    const prevEnd = new Date(curStart.getTime() - 1);
    const prevStart = new Date(prevEnd.getTime() - diffMs);
    const previousCases = allCases.filter(
      (c) => c.date && c.date >= prevStart && c.date <= prevEnd,
    );
    return detectSectorAnomalies(
      displayCases,
      previousCases,
      dateRange,
      anomalyThresholds,
    );
  }, [displayCases, allCases, dateRange, anomalyThresholds]);
  ...
  const dataWarnings = useMemo(() => {
    ...
    allCases.forEach((c) => {
      if (!c.engineer) noEng++;
      if (!c.date) noDate++;
      if (c.tat > KpiConfig.tatAnomalyThreshold) highTat++;
      if (c.revenue < 0) negRev++;
      if (idSeen.has(c.id)) dupeIds.add(c.id);
      else idSeen.add(c.id);
    });
    if (noEng > 0)
      warns.push({ level: "warning", msg: `${noEng} 筆工單缺少工程師欄位` });
    if (noDate > 0)
      warns.push({ level: "warning", msg: `${noDate} 筆工單缺少完成日期` });
    if (highTat > 0)
      warns.push({
        level: "caution",
        msg: `${highTat} 筆工單 TAT 超過 ${KpiConfig.tatAnomalyThreshold} 天（可能異常）`,
      });
    if (negRev > 0)
      warns.push({ level: "caution", msg: `${negRev} 筆工單收費金額為負值` });
    if (dupeIds.size > 0)
      warns.push({
        level: "info",
        msg: `${dupeIds.size} 筆重複工單號碼（已合併處理）`,
      });
    return warns;
  }, [allCases]);
